...
const mockPaymentMethods = [
  ...
  { id: '2', type: 'mastercard', last4: '8888', expiryMonth: 8, expiryYear: 2026, isDefault: false },
];

const mockWallets = [
  { id: '1', name: 'Phantom', address: '7xKXtg2CW87d9wdcxdwxK', connected: true, balance: '12.45 SOL' },
  { id: '2', name: 'Solflare', address: '5yHfEyrjRjhr6T9S', connected: false, balance: '0 SOL' },
];
...
export const SettingsPage: React.FC = () => {
  ...
  const renderNotificationSettings = () => (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {[
          { key: 'email', icon: <Mail className="w-4 h-4" />, label: 'Email Notifications', desc: 'Receive notifications via email' },
          { key: 'push', icon: <Bell className="w-4 h-4" />, label: 'Push Notifications', desc: 'Browser push notifications' },
          { key: 'sms', icon: <Phone className="w-4 h-4" />, label: 'SMS Notifications', desc: 'Text message notifications' },
          { key: 'marketing', icon: <MessageSquare className="w-4 h-4" />, label: 'Marketing Communications', desc: 'Product updates and promotions' },
          ...
          <div key={key} className="flex items-center justify-between p-4 border border-gray-200 rounded-lg">
            ...
            <button
              onClick={() => updateNestedSettings('notifications', key, !(settings.notifications as any)[key])}
              className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 ${
                (settings.notifications as any)[key] ? 'bg-purple-600' : 'bg-gray-200'
              }`}
              aria-label={`Toggle ${label}`}
            >
              <span
                className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                  (settings.notifications as any)[key] ? 'translate-x-6' : 'translate-x-1'
                }`}
              />
            </button>
          </div>
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {[
          ...
          { key: 'allowContact', label: 'Allow Direct Contact', desc: 'Allow other users to contact you directly' },
        ].map(({ key, label, desc }) => (
          <div key={key} className="flex items-center justify-between p-4 border border-gray-200 rounded-lg">
            <div>
              <p className="font-medium text-gray-900">{label}</p>
            ...
            <button
              onClick={() => updateNestedSettings('privacy', key, !(settings.privacy as any)[key])}
              className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 ${
            ...
            >
              <span
                className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                  (settings.privacy as any)[key] ? 'translate-x-6' : 'translate-x-1'
                }`}
              />
            </button>
          </div>
  ...
  const renderPaymentMethods = () => (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button variant="primary" size="sm">Add New Method</Button>
      </div>
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">
                  ...
                  {method.isDefault && <span className="ml-2 px-2 py-1 bg-green-100 text-green-700 text-xs rounded-full">Default</span>}
                </p>
              </div>
            </div>
  ...
  const renderContent = () => {
    switch (activeSection) {
      ...
      case 'wallets': return renderWalletManagement();
        ...
        >
          ...
          >
            ...
            <p className="text-gray-700 mb-6">
              ...
              including your profile, transactions, and connected wallets.
            </p>
            <div className="flex justify-end space-x-3">
